...
class WeatherForecastPage extends Component {
	...
	componentDidMount() {
		...
		// set the first day's data as the default data in weather line chart
		if (this.checkIfCityExist()) {
			var cityInfo = this.getCityInfo();
			axios.get(`https://api.openweathermap.org/data/2.5/forecast?id=${cityInfo.id}&units=imperial&appid=${APIkey}`)
				.then(response => {
